
#include <check.h>
#include <stdlib.h>

#if defined(UNITY_TEST)
#define CSERIO_IMPLEMENTATION
#endif

#include "../cserio.h"

#include "suites.h"

#define OUTPUT_MODE     CK_NORMAL


int main() {
    int number_failed = 0;

    Suite* core_s; 
    core_s = core_suite();
    SRunner* core_sr = srunner_create(core_s);
    srunner_run_all(core_sr, OUTPUT_MODE);
    number_failed = srunner_ntests_failed(core_sr);
    srunner_free(core_sr);

    Suite* create_memory_s; 
    create_memory_s = create_memory_suite();
    SRunner* create_memory_sr = srunner_create(create_memory_s);
    srunner_run_all(create_memory_sr, OUTPUT_MODE);
    number_failed = srunner_ntests_failed(create_memory_sr);
    srunner_free(create_memory_sr);

    Suite* open_view_s; 
    open_view_s = open_view_suite();
    SRunner* open_view_sr = srunner_create(open_view_s);
    srunner_run_all(open_view_sr, OUTPUT_MODE);
    number_failed = srunner_ntests_failed(open_view_sr);
    srunner_free(open_view_sr);

    Suite* open_memory_s; 
    open_memory_s = open_memory_suite();
    SRunner* open_memory_sr = srunner_create(open_memory_s);
    srunner_run_all(open_memory_sr, OUTPUT_MODE);
    number_failed = srunner_ntests_failed(open_memory_sr);
    srunner_free(open_memory_sr);

    Suite* create_file_s; 
    create_file_s = create_file_suite();
    SRunner* create_file_sr = srunner_create(create_file_s);
    srunner_run_all(create_file_sr, OUTPUT_MODE);
    number_failed = srunner_ntests_failed(create_file_sr);
    srunner_free(create_file_sr);

    Suite* open_file_s; 
    open_file_s = open_file_suite();
    SRunner* open_file_sr = srunner_create(open_file_s);
    srunner_run_all(open_file_sr, OUTPUT_MODE);
    number_failed = srunner_ntests_failed(open_file_sr);
    srunner_free(open_file_sr);

    Suite* header_read_s; 
    header_read_s = header_read_suite();
    SRunner* header_read_sr = srunner_create(header_read_s);
    srunner_run_all(header_read_sr, OUTPUT_MODE);
    number_failed = srunner_ntests_failed(header_read_sr);
    srunner_free(header_read_sr);

    Suite* header_write_s; 
    header_write_s = header_write_suite();
    SRunner* header_write_sr = srunner_create(header_write_s);
    srunner_run_all(header_write_sr, OUTPUT_MODE);
    number_failed = srunner_ntests_failed(header_write_sr);
    srunner_free(header_write_sr);

    Suite* image_info_s; 
    image_info_s = image_info_suite();
    SRunner* image_info_sr = srunner_create(image_info_s);
    srunner_run_all(image_info_sr, OUTPUT_MODE);
    number_failed = srunner_ntests_failed(image_info_sr);
    srunner_free(image_info_sr);

    Suite* image_read_s; 
    image_read_s = image_read_suite();
    SRunner* image_read_sr = srunner_create(image_read_s);
    srunner_run_all(image_read_sr, OUTPUT_MODE);
    number_failed = srunner_ntests_failed(image_read_sr);
    srunner_free(image_read_sr);

    Suite* image_write_s; 
    image_write_s = image_write_suite();
    SRunner* image_write_sr = srunner_create(image_write_s);
    srunner_run_all(image_write_sr, OUTPUT_MODE);
    number_failed = srunner_ntests_failed(image_write_sr);
    srunner_free(image_write_sr);

    Suite* trlr_read_s; 
    trlr_read_s = trailer_read_suite();
    SRunner* trlr_read_sr = srunner_create(trlr_read_s);
    srunner_run_all(trlr_read_sr, OUTPUT_MODE);
    number_failed = srunner_ntests_failed(trlr_read_sr);
    srunner_free(trlr_read_sr);

    return (number_failed == 0) ? EXIT_SUCCESS : EXIT_FAILURE;
}

